...
class ArduinoDSLGenerator extends AbstractGenerator {
	...
	def void generateExpressionString(Object x, StringBuilder sb) {
		switch x {
			...
			Delta: {
				var variableName = x.attr.component.name + componentIDs.get(x.attr.name.name + x.attr.component.name) +
					"Value"
				sb.append(variableName)
			}
		}
	...
	def String generateAttributeComponentIdConditions(Attribute[] attributes) {
		...
		for (attribute : attributes) {
			sb.
				append('''id.intval == «componentIDs.get(attribute.name.name + attribute.component.name)»«if (i != attributes.length-1) " || " else ""»''')
			i++
		}
...
while (network.available()) {
		...
		«ENDIF»
			...
			if(millis() > «component.name»LastTransfer + «component.name»Rate){
				...
				«FOR rule : input.allContents.filter(Rule).filter[getAttributes(it.condition).map[it.component].contains(component)].toIterable»
					«FOR assignment : rule.body.statements.filter[it instanceof Assignment].map[it as Assignment]»
	...
	def String generateNestedIfStatements(IfStatement root) {
		'''if(«generateExpressions(root.condition.left)» «root.condition.operator» «generateExpressions(root.condition.right)»)
{
